-- SIGAA Hub UFBA
-- Aumenta o limite de publicações bem-sucedidas de links:
-- 2 -> 10 links em uma janela móvel de 24 horas por reporter_fingerprint.
--
-- O limite de tentativas de add_link_secure permanece em 5 por hora.
-- Esta migration não altera Turnstile, confirmação de e-mail nem RLS.
...
begin
  ...
  if v_successes_last_day >= 10 then
    return 'rate_limited';
  end if;
...
comment on function public.add_link_secure(
  ...
  'Publica o convite após confirmação de e-mail. Limite: 10 publicações bem-sucedidas por fingerprint em 24 horas. Executável somente pelo backend privilegiado.';
